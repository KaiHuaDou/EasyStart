...
public partial class CreateApp : Window, IEditor<AppTile>
{
    ...
    public CreateApp(AppTile? t)
    {
        ...
        colorPicker.SelectedColor = (Item?.TileColor as SolidColorBrush)?.Color ?? Colors.Transparent;
    ...
    {
        Item?.FontSize = double.TryParse(fontBox.Text, out var result)
    ...
    {
        Item?.ImageShadow = imageShadowBox.IsChecked == true;
    }
    ...
    private void PathChanged(object o, RoutedEventArgs e)
    {
        try
        {
            ...
            var name = Path.GetFileNameWithoutExtension(Item?.AppPath) ?? "Application";
    ...
    {
        if (Utils.TrySelectFile(out var fileName, "exe"))
        {
    ...
    {
        if (Utils.TrySelectFile(out var fileName, "exe+img"))
        {
